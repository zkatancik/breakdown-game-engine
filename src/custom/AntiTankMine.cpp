...
/**
 * A component exclusive to AntiTankMine, which uses a counter to animate its
 * explosion, and then removes itself from the level
 */
class ExplodeAndRemoveComponent : public GenericComponent {
 public:
  ExplodeAndRemoveComponent(GameObject& gameObject,
                            std::shared_ptr<TextureRenderComponent> renderer)
      : GenericComponent(gameObject), mRenderer(std::move(renderer)) {
...
AntiTankMine::AntiTankMine(Level& level, float x, float y, Vector2D<int> bs)
  ...
  renderer_->setTexture(ResourceManager::getInstance().getTexture(
      "TD2D/Sprites/Towers/cpix_towers/49.png"));
            ...
            if (!mHasExploded) {
              Mix_PlayChannel(1,
                              ResourceManager::getInstance().getChunk(
                                  "TD2D/Audio/Explosions/Explosion1.mp3"),
                              0);
              addGenericComponent(std::make_shared<ExplodeAndRemoveComponent>(
                  *this, renderer_));
              mHasExploded = true;
            }
          });

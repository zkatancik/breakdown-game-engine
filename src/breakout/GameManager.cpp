...
void GameManager::movePaddleLeft(uint32_t deltaT, int sw) {
  if (isGameStarted()) {
    paddle->MoveLeft(deltaT, sw);
  }
}

void GameManager::movePaddleRight(uint32_t deltaT, int sw) {
  if (isGameStarted()) {
    paddle->MoveRight(deltaT, sw);
  }
}
...
int GameManager::getLivesVal() { return lives; }
...
int GameManager::getNumBlocks() { return numBlocks; }

int GameManager::getLevelVal() { return level; }
...
void GameManager::startGame() {
  ...
  LevelData levelData;
  ResourceManager::getInstance().loadLevel(&levelData, level);

...
Paddle::Paddle(Level& level, float sw, float sh, float width, float speed) : GameObject(level, sw / 2 - (width / 2), sh - 50, width, 24, PaddleTag) {
  ...
  addGenericComponent(
      std::make_shared<PerformHookOnCollideComponent>(*this, BallTag, [&] (Level&, const std::shared_ptr<GameObject>&) {
        Mix_PlayChannel(0, ResourceManager::getInstance().getChunk("2DBreakout/SFX/PaddleBorderHit_SFX.mp3"),0);
      }));
}

...
bool ResourceManager::startUp(SDL_Renderer *renderer) {
  if (!hasStarted_) {
    ...
    if (missing_texture_ == nullptr)
      return false;
    if (SDL_SetRenderTarget(renderer_, missing_texture_) == -1)
      return false;
    if (SDL_SetRenderDrawColor(renderer_, 0xFF, 0x00, 0xFF, 0xFF) == -1)
      return false;
    if (SDL_RenderClear(renderer_) == -1)
      return false;
    if (SDL_SetRenderTarget(renderer_, nullptr) == -1)
      return false;
    hasStarted_ = true;
  }
  return hasStarted_;
...
bool ResourceManager::hasStarted() {
  return hasStarted_;
}
...
SDL_Texture *ResourceManager::getBMPTexture(const std::string &filename) {
  const filesystem::path resPath = getResourcePath("2DBreakout_Graphics");
...
SDL_Texture *ResourceManager::getTextTexture(const std::string &message,
  ...
  const filesystem::path resPath = getResourcePath("2DBreakout_Fonts");
...
Mix_Music *ResourceManager::getMusic(const std::string &path) {
  const filesystem::path resPath = getResourcePath("2DBreakout_SFX");
...
void ResourceManager::loadLevel(LevelData * levelData, int level) {
  ...
  const filesystem::path resPath = getResourcePath("2DBreakout_Levels");
...
std::string ResourceManager::getTranslation(const std::string &message, Language language) {
  auto it = localization_.find(message);
  if (it == localization_.end()) {
    std::cerr << "Couldn't find " << message << "in the localization database." << std::endl;
    return message;
  }
  if (language == Language::ENGLISH)
    ...
    if (message_it == it->second.end()) {
      std::cerr << "Message " << message << "doesn't have translation in " << language << std::endl;
      return message;
    }
